...
**When this applies:** integrating with `~/.pandoc` — the contractually central asset repo the app validates, reads, and partially manages. Audited 2026-06-13; it is a git repo with its own AGENTS.md/HANDOFF.md.
...
**Environment contract (justfile):** `TEXINPUTS=.:~/.pandoc/styles//:~/.pandoc/macros//:~/.pandoc/config//:` ; `FIGURES_DIR` default `~/figures`; `SVG_DIR` default `$FIGURES_DIR/rendered`; `PANDOC_DOC_PATH` set per render. Note the figures-dir ambiguity across sources: `.pandoc` justfile says `~/figures`, an old app config defaulted `~/.pandoc/figures` — the app's config names ONE; verify with the user which is canonical before encoding a value.

**Formatter quirks (HANDOFF.md):** pandoc percent-encodes link targets containing spaces and rewrites SoftBreak→Space (destroying semantic line breaks) — why FORMATTER_SPEC_V2 abandoned "pandoc as final emitter" for a source-preserving protect/restore design with fixture-first tests, wikilinks/math opaque, and `:::` as a fixed delimiter. Any app feature that round-trips the document through pandoc inherits these hazards.
