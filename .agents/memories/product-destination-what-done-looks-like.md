...
**Phase 2 (deliberately deferred, do not build natively):** text expansion, autocomplete, concealing — these either use or are heavily mined from Firenvim, so standard nvim plugins (quicktex-class) own them instead of app code ([Lineage: Vim Live-TeXing Setup](lineage-vim-live-texing-setup)).
