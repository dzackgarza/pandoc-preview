...
| ~~Shared `.tikzstyles` + `.tikzdefs` palette for the global figures dir~~ **SUPERSEDED 2026-06-20 (issue #2):** there is no app-injected figures-dir palette. Shared tikz styles/defs live in the `~/.pandoc/styles` tree and are owned by the `standalone-tikz.tex` preamble (`\usepackage{dzg-tikz}`); tikz rendering is the standard render with that user-owned template. See wiki `Pandoc-Command-Model`. | resolved by doctrine | — |
