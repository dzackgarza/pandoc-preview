...
### Milestone — Phase C: citations (P84; P85–P89 reserved)

P84 (added 2026-06-18) covers the Phase C sub-milestone C1 "bibliography as a single config-declared source", authored from [[phase-c-citations]] (the 2026-06-18 RATIFIED decision). It is the foundation the rest of Phase C consumes: the bibliography the app cites against is one user-controlled config value, read by both the frontend and the preview, validated to exist at load. It is an exact externally-observable happy-path state, driven by the real app via the `tauri-plugin-playwright` harness against real config and a real `.bib`/CSL file. It is admissible only if it FAILS on a plausibly broken app. P85–P89 are reserved for the remaining Phase-C sub-milestones (C2–C5) and are not specified until ratified.
