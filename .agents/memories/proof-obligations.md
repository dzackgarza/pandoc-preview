...
### Milestone — Phase C: citations (P84–P88; P89 reserved)
...
P88 (added 2026-06-18) covers the Phase C sub-milestone C4 "per-file `bibliography:` override", authored from [[phase-c-citations]]. It refines P84's single config-declared bibliography into a per-file-overridable source: a document may declare its own bibliography in its YAML frontmatter, and that file-local declaration takes precedence for that document while the global config bibliography remains the source for documents that do not. It is an exact externally-observable happy-path state, driven by the real app via the `tauri-plugin-playwright` harness against real config and a real sibling `.bib` file carrying a key absent from the global config bibliography. It is admissible only if it FAILS on a plausibly broken app. P89 is reserved for the remaining Phase-C sub-milestone (C5) and is not specified until ratified.
