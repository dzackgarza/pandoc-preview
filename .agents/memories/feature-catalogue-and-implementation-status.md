...
- [ ] Zotero citation insertion: invoke Zotero's own CAYW popup from the editor and insert the returned citation at cursor (plugin-side; the app may hard-require a Better BibTeX export to function — missing/misconfigured export is a fatal setup failure, not degraded mode)

## Tier 5 — Editor experience (phase 2; do not build natively before the Firenvim decision)

- [ ] Firenvim embedding decision, then: snippets/text expansion (quicktex-class, 281-entry dict), conceals, theorem-keyword syntax, folding ([Editor Experience Targets: Conceals, Folding, Expansion](editor-experience-targets-conceals-folding-expansion), [Lineage: Vim Live-TeXing Setup](lineage-vim-live-texing-setup))

## Tier 6 — Wishlist (post-MVP; mine Obsidian plugins for reference implementations first)
