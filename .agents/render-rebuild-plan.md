...
- **2026-06-20: Tier 3 workspace + the full phase-A..h competitive-parity program COMPLETE — all merged to `main`.** Since the 06-17 entry the work shifted from the render-rebuild "Milestones/Tiers" framing to the user-authored phase plans in `.agents/plans/phase-{a..h}-*.md` (the durable decisions + parity evidence live in [[competitive-parity-roadmap]]). Each phase ran the role-separated blind-TDD pipeline via the Workflow tool (independent obligation/test/impl/review agents; RED committed before GREEN; adversarial reviewers empowered to REJECT), with a full serial proof gate before each milestone merge.
  Obligation ledger advanced P70→P123 (the canonical per-obligation record is [[proof-obligations]]; per-phase resume state is each plan's "Status / resume here"):
  - **Phase A — lint fast-feedback (P70–P74).** Typographic/math-balance lint, user regex rules, in-document suppression, structured compile-log jump-to-source.
    (P75 real-ChkTeX-on-transient-`.tex` **HELD**, P76 reserved.)
  ...
  - **Phase F — PDF preview / Gummi parity (P107–P111, P113).** Live pdf.js preview of the real compiled PDF, build-intermediate isolation, multi-pass latexmk reference resolution, auto/manual + fast/full controls, structured warnings-vs-errors Problems, reveal.js slides preview.
    (P112 struck.)
  ...
  - **Phase H — QOL / writing-comfort (P120–P123).** Comfort modes (typewriter/distraction-free/readability), three-way view toggle, batch multi-format export, reading-time metric.
    Readability folded from the P124 spare.
    **Terminal phase** (see `.agents/plans/phase-h-qol.md` for the per-feature record + the merge `6f0e6d9`).
