...
- **2026-06-20: PHASE H COMPLETE — merged to `main` (fast-forward `6f0e6d9`), pushed.** All four features blind-TDD via Workflow (separate obligation/test/impl/review agents) and adversarially ACCEPTED:
  ...
  - **H.4 — reading-time metric (P123, `p129`).** `StatusBar.svelte` shows a `{ceil(words/reading_wpm)} min read` sibling span.
    REJECT→remediated: the first `p129` witness (54/254 words) could not discriminate `wpm=137` from a hardcoded 200/250 (same `ceil`); strengthened to a word count where `ceil(words/137)` is unique vs {100,200,250,300}, with explicit `not.toBe` guards — a hardcoded divisor now fails.
  - **H.3 — batch export (P122, `p130`).** `App.svelte:exportAll(baseDir)` loops the existing per-plugin `exportToPath` over every discovered `category="export"` plugin under the P47 save-gate; writes N real non-empty files (html + pdf) in one action.
    No new export mechanism.
  - **H.1 — comfort modes (P120, `p131`).** typewriter (centered caret), distraction-free (hide chrome), readability (sentence marks) as CM6 `Compartment`s + an App-shell CSS state + an `[editor.comfort]` sub-table.
    **Readability folded in from the reserved P124 spare** so all three modes carry DOM-observed proof (P124 now unused).
    REJECT→remediated: the first impl used forbidden `#[serde(default)]` soft-defaults (fail-open) on the comfort fields; made `[editor.comfort]` genuinely required (no serde default), emitted all-false in first-run.sh + every provision-proof.sh `[editor]` block, flipped the `p131` baseline from table-absent to present-all-false, and removed the frontend `?? false` seeds.
