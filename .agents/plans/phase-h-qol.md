...
- **2026-06-20: decisions RATIFIED (controller, "execute all phases, no stops"); Phases A–G shipped to main.** Executing on branch `phase-h-qol` (the LAST parity phase).
  ...
  - **H.6 (autocorrect / magic quotes) is NOT BUILT** — a correctness hazard (smart-quote substitution corrupting `"` inside `\text{}` / `$…$` LaTeX-bearing source), per the roadmap's standing caution and this plan's disposition.
    No obligation.
    Recorded so its absence is not mistaken for an oversight.
  - Near-zero owned code: lean on published CM6 extensions (typewriter scroll-margin, readability decorations), the existing dockview `SplitviewComponent` visibility API, the existing `exportToPath` loop, and the existing `wordCount` state — refine, don't greenfield.
    New config options are `deny_unknown_fields`-validated + XDG-round-tripped (P9). The view toggle HIDES a dockview pane, never rebuilds the splitview (P13/P15 ratio preserved).
  - Each blind-TDD via Workflow; full-suite gate before merge.

- **2026-06-20: PHASE H COMPLETE — merged to `main` (fast-forward `6f0e6d9`), pushed.** All four features blind-TDD via Workflow (separate obligation/test/impl/review agents) and adversarially ACCEPTED:
  - **H.2 — three-way view toggle (P121, `p128`).** `dockview.ts:setView(mode)` toggles pane visibility via `component.setVisible` (NOT a rebuild), so the split ratio survives a round-trip; `view_mode` persisted in `[editor]`.
  - **H.4 — reading-time metric (P123, `p129`).** `StatusBar.svelte` shows a `{ceil(words/reading_wpm)} min read` sibling span. REJECT→remediated: the first `p129` witness (54/254 words) could not discriminate `wpm=137` from a hardcoded 200/250 (same `ceil`); strengthened to a word count where `ceil(words/137)` is unique vs {100,200,250,300}, with explicit `not.toBe` guards — a hardcoded divisor now fails.
  - **H.3 — batch export (P122, `p130`).** `App.svelte:exportAll(baseDir)` loops the existing per-plugin `exportToPath` over every discovered `category="export"` plugin under the P47 save-gate; writes N real non-empty files (html + pdf) in one action. No new export mechanism.
  - **H.1 — comfort modes (P120, `p131`).** typewriter (centered caret), distraction-free (hide chrome), readability (sentence marks) as CM6 `Compartment`s + an App-shell CSS state + an `[editor.comfort]` sub-table. **Readability folded in from the reserved P124 spare** so all three modes carry DOM-observed proof (P124 now unused). REJECT→remediated: the first impl used forbidden `#[serde(default)]` soft-defaults (fail-open) on the comfort fields; made `[editor.comfort]` genuinely required (no serde default), emitted all-false in first-run.sh + every provision-proof.sh `[editor]` block, flipped the `p131` baseline from table-absent to present-all-false, and removed the frontend `?? false` seeds.
  - **Full-suite gate (all 131 specs, serial, single-tenant :88): 130/131 green.** Sole miss = `p17-export-html-offline`, the documented `unshare -rn` sandbox env-block (no user-namespace privilege here), not a defect.
  - Phase H is the **terminal** parity phase — the competitive-parity roadmap ends at H; no Phase I. README subsequently rewritten for consumers (`420e3ca`, per the `writing-readmes` contract).
  - **Open residue (recorded, not Phase-H scope):** the `sourcepos` / scroll-sync reader decision (still deferred per the master plan) gates P75 (HELD) and the Phase-F bidirectional SyncTeX click-jump; H.6 autocorrect stays unbuilt (hazard); see the master handoff `.agents/render-rebuild-plan.md` "Status / resume here" and the session task list for #5/#14/#15/#16.
