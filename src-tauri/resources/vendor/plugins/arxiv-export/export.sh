...
# ── 3b. (removed) Inline-tikz externalization ──────────────────────────────────
# Tikz is no longer inline-compiled inside a markdown document: a tikz figure is a
# FILE rendered by the tikz-renderer. A markdown/latex source therefore carries no
# inline tikzpicture/tikzcd env for the bundle to externalize, so there is nothing
# to precompile here. (If a future cell embeds tikz files, the bundle references the
# already-rendered figure files; it does not run a tikz compiler.)
