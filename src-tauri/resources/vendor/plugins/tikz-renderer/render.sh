...
# Wrap the source in the user-owned template at its single QTikz `<>` marker (the
# TemplateReplaceText convention). The marker is plain text, not a pandoc $var$,
# so source carrying $ math passes through uninterpreted.
...
if "<>" not in template:
    sys.exit("tikz-renderer: template lacks the <> source marker: " + template_path)
open(out_path, "w", encoding="utf-8").write(template.replace("<>", body, 1))
PY
